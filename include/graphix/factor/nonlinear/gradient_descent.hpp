 ...
 * @brief Gradient descent optimizer - thin wrapper around OptinumGradientDescent
 *
 * This file provides backward-compatible API for the GradientDescentOptimizer
...
namespace graphix::factor {
     ...
     *
     * This is a thin wrapper around OptinumGradientDescent that provides
    ...
    class GradientDescentOptimizer {
      public:
        ...
        inline Result optimize(const Graph<NonlinearFactor> &graph, const Values &initial) const {
            ...
            OptinumGradientDescent optinum_gd;
            optinum_gd.step_size = params_.step_size;
            optinum_gd.max_iterations = static_cast<std::size_t>(params_.max_iterations);
            optinum_gd.tolerance = params_.tolerance;
            optinum_gd.h = params_.h;
            optinum_gd.verbose = params_.verbose;
            optinum_gd.use_adam = params_.use_adaptive_lr;
            optinum_gd.adam_beta1 = params_.adam_beta1;
            optinum_gd.adam_beta2 = params_.adam_beta2;
            optinum_gd.adam_epsilon = params_.adam_epsilon;

            // Delegate to optinum implementation
            auto optinum_result = optinum_gd.optimize(graph, initial);

            // Convert result to our format
            return Result(optinum_result.values, optinum_result.final_error, optinum_result.iterations,
                          optinum_result.converged, optinum_result.gradient_norm);
        }
