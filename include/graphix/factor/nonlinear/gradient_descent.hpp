...
namespace graphix::factor {
    ...
    namespace detail {
        ...
        class GradientCache {
          public:
            ...
            inline void add_scalar_gradient(Key key, double grad) {
                gradients[key] = grad;
                norm += grad * grad;
            }
            ...
            inline const GradientValue &get(Key key) const { return gradients.at(key); }

            inline bool has(Key key) const { return gradients.find(key) != gradients.end(); }
        };
        ...
        class AdamState {
          public:
            ...
            inline void next_step() { ++t; }

            inline void update(Key key, const GradientValue &grad, double beta1, double beta2) {
            ...
            inline GradientValue get_corrected_update(Key key, double beta1, double beta2, double epsilon) const {
                if (t <= 0) {
        ...
        inline GradientCache compute_all_gradients(const Graph<NonlinearFactor> &graph, const Values &values,
                                                   double h) {
            GradientCache cache;
            ...
            for (Key key : keys) {
                ...
                try {
                    ...
                    try {
                        ...
                        throw std::runtime_error("Unsupported type in gradient computation for key " +
                                                 std::to_string(key));
                    }
                }
            }
        ...
        inline Values apply_adam_update(const Values &current, const AdamState &adam, const GradientCache &grad_cache,
        ...
        inline Values apply_gradient_update(const Values &current, const GradientCache &grad_cache, double step_size) {
            Values updated = current;
    ...
    class GradientDescentOptimizer {
      public:
         ...
         */
        inline explicit GradientDescentOptimizer(const Parameters &params) : params_(params) {}
        ...
        inline Result optimize(const Graph<NonlinearFactor> &graph, const Values &initial) const {
            ...
            for (iteration = 0; iteration < params_.max_iterations; ++iteration) {
                ...
                auto grad_cache = detail::compute_all_gradients(graph, current_values, params_.h);
                gnorm = grad_cache.norm;

                if (params_.verbose && (iteration % 100 == 0 || iteration < 10)) {
                    std::cout << "Iteration " << iteration << ": error = " << current_error
                ...
                if (params_.use_adaptive_lr) {
                    ...
                    // Apply Adam update
                    new_values =
                        detail::apply_adam_update(current_values, adam, grad_cache, params_.step_size,
                                                  params_.adam_beta1, params_.adam_beta2, params_.adam_epsilon);
                } else {
                    // Standard gradient descent
                    new_values = detail::apply_gradient_update(current_values, grad_cache, params_.step_size);
                }
                ...
                while (new_error > current_error && backtrack_count < max_backtrack) {
                    ...
                    if (params_.use_adaptive_lr) {
                        new_values =
                            detail::apply_adam_update(current_values, adam, grad_cache, step, params_.adam_beta1,
                                                      params_.adam_beta2, params_.adam_epsilon);
                    } else {
                        new_values = detail::apply_gradient_update(current_values, grad_cache, step);
                    }
