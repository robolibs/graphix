 ...
 * (outlier measurements) in a 2D SLAM problem.
 */
...
int main() {
    ...
    graph.add(std::make_shared<Vec3BetweenFactor>(X(0), X(1), Vec3d{2.0, 0.0, 0.0}, odom_sigma));       // → East
    graph.add(std::make_shared<Vec3BetweenFactor>(X(1), X(2), Vec3d{0.0, 2.0, M_PI / 2}, odom_sigma));  // ↑ North
    graph.add(std::make_shared<Vec3BetweenFactor>(X(2), X(3), Vec3d{-2.0, 0.0, M_PI / 2}, odom_sigma)); // ← West
    graph.add(std::make_shared<Vec3BetweenFactor>(X(3), X(4), Vec3d{0.0, -2.0, M_PI / 2}, odom_sigma)); // ↓ South
    ...
    auto bad_loop = std::make_shared<Vec3BetweenFactor>(X(2), X(0), Vec3d{-0.5, -0.5, -M_PI}, loop_sigma);
    ...
    initial.insert(X(1), Vec3d{2.0, 0.0, 0.0});
    initial.insert(X(2), Vec3d{2.0, 2.0, M_PI / 2});
    initial.insert(X(3), Vec3d{0.0, 2.0, M_PI});
    initial.insert(X(4), Vec3d{0.0, 0.0, 3 * M_PI / 2});
    ...
    Vec3d pose2_no_robust = result_no_robust.values.at<Vec3d>(X(2));
    std::cout << "X(2) optimized position: (" << std::fixed << std::setprecision(3) << pose2_no_robust[0] << ", "
              << pose2_no_robust[1] << ")" << std::endl;
    std::cout << "Expected position:        (2.000, 2.000)" << std::endl;
    std::cout << "Error (mm): "
              << std::sqrt(std::pow(pose2_no_robust[0] - 2.0, 2) + std::pow(pose2_no_robust[1] - 2.0, 2)) * 1000
              << std::endl;
    ...
    auto bad_loop_huber = std::make_shared<Vec3BetweenFactor>(X(2), X(0), Vec3d{-0.5, -0.5, -M_PI}, loop_sigma);
    ...
    Vec3d pose2_huber = result_huber.values.at<Vec3d>(X(2));
    std::cout << "X(2) optimized position: (" << std::fixed << std::setprecision(3) << pose2_huber[0] << ", "
              << pose2_huber[1] << ")" << std::endl;
    std::cout << "Expected position:        (2.000, 2.000)" << std::endl;
    std::cout << "Error (mm): "
              << std::sqrt(std::pow(pose2_huber[0] - 2.0, 2) + std::pow(pose2_huber[1] - 2.0, 2)) * 1000 << std::endl;
    ...
    auto bad_loop_cauchy = std::make_shared<Vec3BetweenFactor>(X(2), X(0), Vec3d{-0.5, -0.5, -M_PI}, loop_sigma);
    ...
    Vec3d pose2_cauchy = result_cauchy.values.at<Vec3d>(X(2));
    std::cout << "X(2) optimized position: (" << std::fixed << std::setprecision(3) << pose2_cauchy[0] << ", "
              << pose2_cauchy[1] << ")" << std::endl;
    std::cout << "Expected position:        (2.000, 2.000)" << std::endl;
    std::cout << "Error (mm): "
              << std::sqrt(std::pow(pose2_cauchy[0] - 2.0, 2) + std::pow(pose2_cauchy[1] - 2.0, 2)) * 1000 << std::endl;
    ...
    auto bad_loop_tukey = std::make_shared<Vec3BetweenFactor>(X(2), X(0), Vec3d{-0.5, -0.5, -M_PI}, loop_sigma);
    ...
    Vec3d pose2_tukey = result_tukey.values.at<Vec3d>(X(2));
    std::cout << "X(2) optimized position: (" << std::fixed << std::setprecision(3) << pose2_tukey[0] << ", "
              << pose2_tukey[1] << ")" << std::endl;
    std::cout << "Expected position:        (2.000, 2.000)" << std::endl;
    std::cout << "Error (mm): "
              << std::sqrt(std::pow(pose2_tukey[0] - 2.0, 2) + std::pow(pose2_tukey[1] - 2.0, 2)) * 1000 << std::endl;
    ...
    auto compute_error = [](const Vec3d &pose, double true_x, double true_y) {
        return std::sqrt(std::pow(pose[0] - true_x, 2) + std::pow(pose[1] - true_y, 2)) * 1000;
    };
